...
function gameLoop() {
	...
	if (time) {
		speed = startSpeed * (1 - time / 314150);
		
		document.getElementById('time').innerHTML = (time / 1000).toFixed(2) + 's';
		document.getElementById('speed').innerHTML = (startSpeed / speed).toFixed(2) + 'x';
...
function updateMap(milliseconds) {
	if (milliseconds >= speed * speedMultiplier) {
...
/*function updateForm() {
	...
	xhr.open('GET', 'http://localhost:8088/gethighscore/', true);
	...
	var inputScore = document.getElementsByName('score')[0];
	var inputTime = document.getElementsByName('time')[0];
	var inputSpeed = document.getElementsByName('speed')[0];
	inputScore.value = document.getElementById('score').innerHTML;
	inputTime.value = document.getElementById('time').innerHTML;
	inputSpeed.value = document.getElementById('speed').innerHTML;
}*/